...
export const selectors: SelectorType = {
  isFetching: ({ pickers }: RootState) => pickers.fetching,
  getPendingUser: ({ pickers }: RootState) => pickers.users,
  getFilters: ({ pickers }: RootState) => pickers.filters,
  getFiltersExtra: ({ pickers }: RootState) => pickers.filtersExtra,
  getFiltersExtraSeeMore: ({ pickers }: RootState) =>
    pickers.filtersExtraSeeMore,
  getSeeMore: ({ pickers }: RootState) => pickers.seeMore,
  getSizePage: ({ pickers }: RootState) => pickers.sizePage,
  getActualPage: ({ pickers }: RootState) => pickers.actualPage,
};
...
const reducer = (
  state: PickerStateType = INITIAL_STATE,
  action: ActionType
) => {
  switch (action.type) {
